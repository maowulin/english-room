...
describe("HttpRoomClient", () => {
  ...
  it("maps sessions, snapshots, reports and retry requests to the FastAPI contract", async () => {
    ...
    fetcher
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ room: { room_id: "r1", room_code: "4827", title: "雾港疑云", status: "ended", version: 5, members: [] }, score_jobs: [{ score_job_id: "s1", player_id: "p1", status: "success", scores: { overall: 86, pronunciation: 88, fluency: 82 }, recognized_text: "Hello" }] }) } as Response)
