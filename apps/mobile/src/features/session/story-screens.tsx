...
export function LiveScreen({
  ...
  const isMyTurn = Boolean(localPlayerId && currentSpeakerPlayerId === localPlayerId);
  const currentSpeakerName = members.find((member) => member.playerId === currentSpeakerPlayerId)?.nickname;
  const endAllowed = canEnd && allTurnsCompleted;
  const microphoneLocked = Boolean(localPlayerId && (currentSpeakerPlayerId || allTurnsCompleted) && !isMyTurn);
    ...
    <SafeAreaView style={styles.liveSafe}>
      <View style={liveRootStyle} testID="live-screen-root">
        ...
        <View style={styles.liveTop}>
          <Pressable accessibilityLabel="Leave room" disabled={!endAllowed || busy} onPress={onEnd} style={[styles.liveHeaderButton, (!endAllowed || busy) && styles.ctaDisabled]}><AppIcon color="#FFF" name="arrow-left" size={21} /></Pressable>
          <View>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: LIVE_SCROLL_CONTENT_PADDING }} showsVerticalScrollIndicator={false} style={[styles.liveBody, fidelityStyles.liveBody]} testID="live-scroll">
          ...
          <View style={styles.turnStatus} testID="live-turn-status">
            <Text style={styles.turnStatusTitle}>
              {allTurnsCompleted ? "All turns complete" : isMyTurn ? "Your turn" : `${currentSpeakerName ?? "Another player"}'s turn`}
            </Text>
            <Text style={styles.turnStatusDetail}>{completedTurnCount} of {members.length || 1} turns complete</Text>
          </View>
          {isMyTurn && !allTurnsCompleted ? (
            <Pressable
              accessibilityLabel="Finish turn"
              disabled={busy || realAudioDisabled}
              onPress={onCompleteTurn}
              style={[styles.finishTurn, (busy || realAudioDisabled) && styles.ctaDisabled]}
              testID="finish-turn-button"
            >
              <View style={styles.inlineButtonContent}><AppIcon color="#174638" name="check" size={17} /><Text style={styles.finishTurnText}>Finish turn</Text></View>
            </Pressable>
          ) : null}
        ...
        <View style={[styles.controls, fidelityStyles.controls]} testID="live-controls">
          <Control disabled={realAudioDisabled || microphoneLocked} label={microphoneLocked ? "Mic locked" : muted ? "Unmute" : "Mute"} icon={microphoneLocked || muted ? "mic-off" : "mic"} onPress={toggleMute} />
          ...
          <Control danger disabled={busy || !endAllowed} label={busy ? "Ending…" : "End room"} icon="phone-off" onPress={onEnd} testID="end-room-button" />
        </View>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  turnStatus: { alignItems: "center", backgroundColor: "rgba(255, 246, 218, .1)", borderColor: "rgba(214, 185, 113, .5)", borderRadius: 12, borderWidth: 1, marginTop: 10, padding: 9 },
  turnStatusTitle: { color: "#FFF6DA", fontSize: 14, fontWeight: "800", textAlign: "center" },
  turnStatusDetail: { color: "#D8C58F", fontSize: 11, marginTop: 3, textAlign: "center" },
  finishTurn: { alignItems: "center", backgroundColor: "#E5F0E6", borderColor: "#9CC4A7", borderRadius: 12, borderWidth: 1, marginTop: 10, padding: 13 },
  finishTurnText: { color: "#174638", fontSize: 14, fontWeight: "800" },
