      ...
      {members.map((member, index) => (
        <View key={member.playerId} style={styles.seat}>
          <View style={[styles.seatAvatar, { backgroundColor: memberSeatTone(index) }]}>
            <Text style={styles.avatarInitial}>{member.nickname.slice(0, 1) || "♙"}</Text>
          </View>
          <View style={styles.seatMeta}>
      ...
      {demoPlayers.concat([["", "等待加入", "#DDD7C7"], ["", "等待加入", "#DDD7C7"]]).map(([name, state, tone], index) => (
        <View key={`${name}-${index}`} style={styles.seat}>
          <View style={[styles.seatAvatar, { backgroundColor: tone }]}>
            <Text style={styles.avatarInitial}>{name || "♙"}</Text>
          </View>
          <View style={styles.seatMeta}>
          ...
          {name ? <Text style={styles.mic}>♩</Text> : null}
        </View>
      ))}
    </View>
  );
...
function WaitingMediaNotice({ mediaState }: { mediaState: MediaUiState }) {
  ...
  const message =
    mediaState.permission === "denied"
      ...
      : mediaState.permission === "requesting"
        ? "正在请求麦克风权限"
...
export function LobbyScreen({
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.lobby}><Header title="" /><Text style={mediaState.mode === "real" ? styles.realChip : styles.demoChip} accessibilityLabel={mediaState.mode === "real" ? "真实语音模式" : "Demo 控制面"}>{mediaState.mode === "real" ? "真实语音模式 · TRTC/SOE" : "Demo / Fake 控制面 · 非真实 TRTC/SOE"}</Text><Text style={styles.lobbyTitle}>用英语进入今晚的故事</Text><Text style={styles.lobbyPrompt}>今晚想练哪一句？</Text><View style={styles.smallRule}><View /><Text>✦</Text><View /></View>
    <View style={styles.portCard}><View style={styles.portMist} /><Text style={styles.portName}>●  雾港疑云</Text><Text style={styles.roomPill}>{mediaState.mode === "real" ? "ROOM LIVE" : "ROOM DEMO"}</Text><View style={styles.ship}><View style={styles.mast}/><View style={styles.hull}/></View>
    {mediaState.mode === "demo" ? (
      <>
        <View style={styles.avatarGroup}><Text>林</Text><Text>M</Text><Text>A</Text><Text>苏</Text><Text>+2</Text></View>
        <Text style={styles.portMeta}>♧  4 / 6 位玩家   |   ◷ 预计 25 分钟</Text>
      </>
    ) : (
      <Text style={styles.portMeta}>♧  真实房间 · 创建或加入后同步成员</Text>
    )}
    </View>
      ...
      <View style={styles.live}>
        ...
        <View style={styles.liveBody}>
          ...
          <View style={styles.signalRow}>
            {realMode ? (
              <View style={styles.signal}><Text>▥</Text><Text>本机 {networkLabel}</Text></View>
            ) : (
        ...
        <View style={styles.controls} testID="live-controls">
          ...
          {realMode ? (
            rtcNeedsReconnect ? (
              ...
              <Control disabled icon="•••" label="更多" onPress={() => undefined} />
            )
          ) : (
            <Control label="触发重连" icon="•••" onPress={() => setDemoReconnecting(!demoReconnecting)} testID="reconnect-button" />
          )}
