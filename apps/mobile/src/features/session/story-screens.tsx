...
export function LobbyScreen({
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.lobby}><Header title="" /><Text style={styles.demoChip} accessibilityLabel="Demo 控制面">Demo / Fake 控制面 · 非真实 TRTC/SOE</Text><Text style={styles.lobbyTitle}>用英语进入今晚的故事</Text><Text style={styles.lobbyPrompt}>今晚想练哪一句？</Text><View style={styles.smallRule}><View /><Text>✦</Text><View /></View>
...
export function LiveScreen({ onEnd, busy = false }: { onEnd: () => void; busy?: boolean }) {
  ...
  return (
    <SafeAreaView style={styles.liveSafe}>
      <View style={styles.live}>
        <Text style={styles.liveDemo} accessibilityLabel="Demo 控制面">Demo / Fake RTC · 非真实语音</Text>
        <View style={styles.liveTop}>
          <Text style={styles.liveBack}>‹</Text>
          <View>
            <Text style={styles.liveRoom}>雾港疑云</Text>
            <Text style={styles.livePractice}>正在练习</Text>
            <Text style={styles.liveStatus}>▥  语音进行中{"\n"}18:42</Text>
          </View>
        ...
        <View style={styles.controls} testID="live-controls">
          <Control label={muted ? "打开麦克风" : "静音"} icon="♩" onPress={() => setMuted(!muted)} />
          <Control label={speaker ? "扬声器开" : "扬声器关"} icon="◖" onPress={() => setSpeaker(!speaker)} />
          <Control label="触发重连" icon="•••" onPress={() => setReconnecting(!reconnecting)} testID="reconnect-button" />
...
export function ReportScreen({ error, items, onDone, onRetry }: { error?: string; items: ReportItem[]; onDone: () => void; onRetry: (item: ReportItem) => void }) {
  ...
  const statusText = (status: ReportItem["status"]) => ({ completed: "评分完成", processing: "处理中 预计 20 秒", waiting: "等待评分", failed: "评分失败" }[status]);
